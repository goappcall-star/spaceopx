...
export function DmAttachmentView({ attachment }: { attachment: Attachment }) {
  const [url, setUrl] = useState<string | null>(null);

  useEffect(() => {
    let active = true;
    void dmUploadsService.signedUrl(attachment.path).then((signed) => {
    ...
    <div className="border-border bg-surface mt-1.5 flex max-w-sm items-center gap-3 rounded-lg border p-2.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-muted-foreground text-xs">
          {attachment.mime || "arquivo"} · {formatBytes(attachment.size)}
        </p>
      </div>
